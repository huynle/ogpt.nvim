...
function EditWindow:init(options)
  options = vim.tbl_deep_extend("keep", options or {}, Config.options.Edit_window)
  ...
  EditWindow.super.init(self, options)
end

function EditWindow:update_popup_size(opts)
...
function EditWindow:calculate_size(opts)
  opts = opts or {}
  -- compute size
...
function EditWindow:mount(opts)
  EditWindow.super.mount(self)

  self:update_popup_size(opts)

  -- close
  local keys = Config.options.Edit_window.keymaps.close
  ...
  self:map("n", Config.options.Edit_window.keymaps.accept, function()
  ...
  self:map("n", Config.options.Edit_window.keymaps.prepend, function()
    local _lines = vim.api.nvim_buf_get_lines(self.bufnr, 0, -1, false)
    table.insert(_lines, "")
    table.insert(_lines, "")
    vim.api.nvim_buf_set_text(
  ...
  self:map("n", Config.options.Edit_window.keymaps.append, function()
    local _lines = vim.api.nvim_buf_get_lines(self.bufnr, 0, -1, false)
  ...
  self:map("n", Config.options.Edit_window.keymaps.yank_code, function()
    local _lines = vim.api.nvim_buf_get_lines(self.bufnr, 0, -1, false)
  ...
  self:map("n", Config.options.Edit_window.keymaps.yank_to_register, function()
    local _lines = vim.api.nvim_buf_get_lines(self.bufnr, 0, -1, false)
